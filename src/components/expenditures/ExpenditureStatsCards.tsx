...
import type { ExpenditureStats } from "@/types/expenditure-types";
...
// <== EXPENDITURE STATS CARDS PROPS ==>
interface ExpenditureStatsCardsProps {
  // <== STATS DATA FROM API ==>
  stats: ExpenditureStats | undefined;
  // <== LOADING STATE ==>
  isLoading: boolean;
}

// <== EXPENDITURE STATS CARDS COMPONENT ==>
const ExpenditureStatsCards = memo(
  ({ stats, isLoading }: ExpenditureStatsCardsProps) => {
    ...
    const cards: StatCard[] = [
      ...
      {
        label: "Avg. Spend",
        value: `₨${Math.round(stats?.avgAmount ?? 0).toLocaleString()}`,
        icon: TrendingDown,
        trend: "down",
        colorClass: "bg-red-500/10 text-red-600 dark:text-red-400",
      },
      ...
      {
        label: "Highest Entry",
        value: `₨${(stats?.highestAmount ?? 0).toLocaleString()}`,
        icon: Wallet,
        trend: "down",
        colorClass: "bg-red-500/10 text-red-600 dark:text-red-400",
      },
...
ExpenditureStatsCards.displayName = "ExpenditureStatsCards";

// <== EXPORT ==>
export default ExpenditureStatsCards;
