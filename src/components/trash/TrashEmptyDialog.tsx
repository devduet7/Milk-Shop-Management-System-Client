...
// <== TRASH EMPTY DIALOG PROPS ==>
interface TrashEmptyDialogProps {
  // <== DIALOG OPEN STATE ==>
  open: boolean;
  // <== LABEL OF THE SCOPE BEING EMPTIED ==>
  scopeLabel: string;
...
// <== TRASH EMPTY DIALOG COMPONENT ==>
const TrashEmptyDialog = memo(
  ({
    open,
    scopeLabel,
    isPending,
    ...
    // RETURNING EMPTY TRASH CONFIRMATION DIALOG
    return (
      <Dialog
        open={open}
        onOpenChange={(v) => {
        ...
        <DialogContent className="flex flex-col p-0 w-[calc(100vw-2rem)] sm:max-w-sm overflow-hidden gap-0">
          ...
          <div className="px-5 py-4">
            <p className="text-sm text-muted-foreground">
              Are you sure you want to permanently delete every item in{" "}
              <span className="font-semibold text-foreground">
                {scopeLabel}
              </span>
              ? None of these items can be recovered afterwards.
            </p>
          </div>
          {/* FIXED FOOTER */}
...
TrashEmptyDialog.displayName = "TrashEmptyDialog";

// <== EXPORT ==>
export default TrashEmptyDialog;
